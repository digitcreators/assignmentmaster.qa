@extends('layouts.web')
@section('title', 'Assignment Master Qatar Pricing - Transparent & Fair Rates')
@section('description', 'Check the pricing at Assignment Master Qatar to see how we balance affordability and quality. Flexible plans designed to support every student’s needs.')
...
<section class="container py-6 px-4 mx-auto" x-data="datatables()" x-cloak>
    <h1 class="text-4xl font-semibold py-8 span-header text-center">Pocket-Friendly Prices, Premium Quality</h1>
                ...
                @foreach($deadlines as $deadline)
                    <tr class="hover:bg-primary-one hover:text-white">
                        ...
                        @foreach($academicLevels as $level)
                            ...
                            @endphp
                            <td class="border border-t border-primary-one days">
                                <span class="px-2 md:px-6 py-2 flex justify-center items-center text-base">{{ $price ? $price->per_page_price . ' QAR' : 'N/A' }}</span>
                            </td>
                        @endforeach
                    </tr>
                @endforeach
